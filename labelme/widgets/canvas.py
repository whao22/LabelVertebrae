...
class Canvas(QtWidgets.QWidget):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.angles = [0,0,0]
        self.flags=None
        self.anglesShow.emit(self.angles)
        self.shapesBackups = []
        self.current = None
    ...
    def mouseMoveEvent(self, ev):
        # if self.current:
        #     print("self.current length is: ",len(self.current))
            ...
            if self.drawing():
                if self.current:
                    ...
                    if self.createMode in ["polygon"]:
                        ...
                        self.finalise()
                    elif self.createMode in ["linestrip"]:
                        self.current.addPoint(self.line[1])
                        self.line[0] = self.current[-1]
                        if int(ev.modifiers()) == QtCore.Qt.ControlModifier:
                            self.finalise()
                    elif self.createMode in ["curve"]:
                        # self.finalise()
                        pass
                    ...
                    if self.createMode=="curve":
                        ps=np.array([[30,20],[50,40],[80,60],[110,80],[110,100],
                            [90,120],[80,140],[50,160],[30,180],[10,200],
                            [30,220],[50,240],[60,260],[80,280],[110,300],[100,320],[90,330]])
                        for p in ps:
                            self.current.addPoint(QtCore.QPointF(*p))
    ...
    def mouseReleaseEvent(self, ev):
        ...
        # 计算cal角且通过mask得到centerline
        if len(self.shapes)>=4:
            cnt=0
            mask_idx,curve_idx=-1,-1
            ...
            if mask_idx!=-1:
                ...
                centerline = getCenterline(self.shapes[mask_idx])
                while self.shapes[curve_idx].points !=[]:
                    self.shapes[curve_idx].popPoint()
                for p in centerline:
                    self.shapes[curve_idx].addPoint(p)
    ...
    def mouseDoubleClickEvent(self, ev):
        ...
        if (
            ...
            # if self.current.shape_type== "polygons":
            #     print("aaaaaaaaaaaaaaaa")
            #     centerline = getCenterline(self.current.points)
            #     newshape=Shape(label="center",shape_type="curve")
            #     for p in centerline:
            #         newshape.addPoint(p)
    ...
    def paintEvent(self, event):
        ...
        for i,shape in enumerate(self.shapes):
            # print(f"{i} shapeaaaa: ",shape.shape_type)
