...
from labelme.utils.Bspline import BSpline
...
class Shape(object):
    ...
    def canAddPoint(self):
        ...
        return self.shape_type in ["linestrip"]
        ...
        if self.points:
            ...
            if self.shape_type == "rectangle":
                ...
                if len(self.points)<=2:
                    ...
                    data=np.zeros((17,2),dtype=np.float)
                    for i,p in enumerate(self.points):
                        data[i][0],data[i][1]=p.y(),p.x()
                    bs=BSpline(h=11,p=3,D=data)
                    uq=np.linspace(0,1,100)
                    re=bs.run(uq)

                    for p in re:
                        p_=QtCore.QPointF(p[1],p[0])
                        line_path.lineTo(p_)

                    # for i, p in enumerate(self.points):
                    #     self.drawVertex(vrtx_path, i)
                    self.drawVertex(vrtx_path, 0)
                    self.drawVertex(vrtx_path, 16)
                    self.fill=False
